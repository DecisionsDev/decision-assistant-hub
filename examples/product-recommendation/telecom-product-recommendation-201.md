...
This policy defines how telecom plans and add-ons are recommended using
...
-   Prioritize eligible offers based on campaigns and operational
    constraints
...
Rules defined earlier may exclude an offer from later evaluation.
...
Example:\
Offer T-300 (Business Secure SIM) is regulated and only allowed in
...
Campaign rules influence priority but NEVER override eligibility or
operational rules.
...
→ Increase visibility for all segments.

### 5.3 Standard Behavior

IF no campaign applies\
→ Offer retains Normal Priority.
...
Example:\
T-400 (Travel Data Pack) has Low stock and must be excluded even during
SummerBoost.
